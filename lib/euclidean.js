...
// Replacing mathjs for lower dependency
function identityMatrix(n) {
    const A = Array(n).fill().map(() => Array(n).fill(0));
    for ( let i = 0; i < n; i ++ ) A[i][i] = 1;
    return A
}
function _invertMatrixSubblock(A,I,offset) {
    let ri = null;
...
function invertMatrix(_A) {
    const I = identityMatrix(_A.length);
    const A = copyMatrix(_A);
    for ( let i = 0; i < _A.length; i ++ ) _invertMatrixSubblock(A,I,i);
    return I;
}
...
function apply_lrrv(lrrv, v) {
  const shifted_v = subtractVectors(v, lrrv.slice(0, 3));
...
function homo2sre(H) {
  ...
  const shift_prime = [H[0][3], H[1][3], H[2][3]];
  const shift = multiplyMatrixVector(transposeMatrix(R), shift_prime);
...
function compose_lrQ(lrQ1, lrQ2) {
  ...
  const location3 = addVectors(
    lrQ1.slice(0, 3),
    apply_quat(invert_quat(q1), lrQ2.slice(0, 3))
  );
  return [...location3, ...q3];
}
